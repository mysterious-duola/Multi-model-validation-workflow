...
- **设置面板**：侧边栏内可实时修改各模型的 API Key、Base URL 和 Model，无需重启
- **历史记录优化**：长文本自动单行截断显示，支持逐条删除和一键清空
- **PDF 导出修复**：换用 TrueType 字体，修复中文导出乱码问题
...
├── packages.txt         # 系统级依赖（云端部署用）
└── 启动科研工作流.bat    # Windows 一键启动脚本
```
